...
generalizability_est <- function(data, 
                                 rho=NA,
                                 m_lib=c('SL.mean','SL.glm','SL.glm.interaction'),
  ...
  # Outcome model
  mu_hat_ests <- mu_hat_est(data,sl.lib=m_lib)
  mu_hat1 <- mu_hat_ests$mu_hat1$pred
  mu_hat0 <- mu_hat_ests$mu_hat0$pred
  
  # Propensity score model
  pi_hat <- pi_hat_est(data,sl.lib=g_lib)$pred
  ...
  kappa_hat <- rho
  if (is.na(rho)) { 
    kappa_hat <- data$val_probs #kappa_hat_est(data)$pred
  }
