# Helper functions for simulation study (complex val extension)
...
gen_data <- function(n, rho, # sample sizes
  ...
  Xmatsamp <- cbind(Xmat,y,Astar)
  val_idx_res <- sampling_vector(Xmatsamp,rho,etas)
...
main_sim <- function(nsim,
  ...
  results <-foreach(s = 1:nsim, .combine = rbind) %dopar% {
    ...
    # Control variates 
    tau1cvres <- cv_est_general(data)
    tau1cv <- tau1cvres$ATE
    var1cv <- tau1cvres$var_hat
    cilowcv <- tau1cv - 1.96*sqrt(var1cv)
    cihicv <- tau1cv + 1.96*sqrt(var1cv)
    
    # Rose + vdL optimal augmentation
    Astar <- data.frame(Astar=df$Astar)
    tau1drcmdres <- drcmd::drcmd(data$Y,
                              data$A,
                              data %>% select(c('X1','X2','X3')),
                              W=Astar,
                              Rprobs=data$val_probs,
                              default_learners=sl.lib)
    tau1drcmd <- tau1drcmdres$results$estimates$psi_hat_ate_direct
    var1drcmd <- tau1drcmdres$results$ses$psi_hat_ate_direct^2
    cilowdrcmd <- tau1drcmd - 1.96*sqrt(var1drcmd)
    cihidrcmd <- tau1drcmd + 1.96*sqrt(var1drcmd)
    ...
    return(
      data.frame(tau1hat=tau1hat,
                      ...
                      var1cv=var1cv,var1drcmd=var1drcmd,
                      cilowcv=cilowcv,cihicv=cihicv,
                      cilowdrcmd=cilowdrcmd,cihidrcmd=cihidrcmd,
