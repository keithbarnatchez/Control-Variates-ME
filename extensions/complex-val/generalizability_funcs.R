...
generalizability_est <- function(data, 
                                 rho=NA,
                                 m_cov_names=c('X1','X2','X3'),
                                 g_cov_names=c('X1','X2','X3')) {
  ...
  # Outcome model
  mu_hat_ests <- mu_hat_est(data,cov_names=m_cov_names)
  mu_hat1 <- mu_hat_ests$mu_hat1$pred
  mu_hat0 <- mu_hat_ests$mu_hat0$pred
  
  # Propensity score model
  pi_hat <- pi_hat_est(data,cov_names=g_cov_names)$pred
...
cv_est_general <- function(data,
  ...
  # Fit sampling probs
  kappa_hat <- data$val_probs # as.double(kappa_hat_est(data)$pred)
  
  # Fit re-weighted outcome regression
  Y = data$Y
  X <- data %>% select(all_of(c(cov_names)))
  A <- data$A
  val_idx <- data$val_idx
  A[val_idx==0] <- 0 # cast NAs to 0 to not throw off superlearner (these obs will always get 0 weight)
  data$A[val_idx==0] <- 0 # cast NAs to 0 to not throw off superlearner (these obs will always get 0 weight)
  Astar <- data$Astar
  
  Xmat = data%>% select(all_of(c(cov_names,'A')))
  Xmain <- data %>% select(all_of(c(cov_names,'A')))
  ...
  # Fit the outcome model
  mu_mod <- SuperLearner::SuperLearner(Y=Y, X=Xmat,
                                       SL.library=sl.lib,
                                       obsWeights=val_idx/kappa_hat,
                                       family=fam)
  XA1 <- Xmain %>% mutate(A=1) 
  mu_hat1 <- as.double(predict(mu_mod, XA1)$pred)
  XA0 <- Xmain %>% mutate(A=0)
  mu_hat0 <- as.double(predict(mu_mod, XA0)$pred)
  
  # Fit re-weighted propenisty scores
  pi_mod <- SuperLearner::SuperLearner(Y=A, X=X,
                                       SL.library=sl.lib,
                                       obsWeights=val_idx/kappa_hat,
                                       family='binomial')
  pi_hat <- as.double(predict(pi_mod,Xmain)$pred)
  
  # Fit error prone outcome regression
  mu_mod_ep <- SuperLearner::SuperLearner(Y=Y,
                                       X=data %>% select(all_of(c(cov_names,'Astar'))),
                                       SL.library=sl.lib,
                                       family=fam)
  mu_hat1_ep <- as.double(predict(mu_mod_ep, Xmain %>% mutate(Astar=1))$pred)
  mu_hat0_ep <- as.double(predict(mu_mod_ep, Xmain %>% mutate(Astar=0))$pred)
  
  # Fit error prone propensity scores
  pi_mod_ep <- SuperLearner::SuperLearner(Y=data$Astar,
                                       X=X,
                                       SL.library=sl.lib,
                                       family='binomial')
  pi_hat_ep <- as.double(predict(pi_mod_ep)$pred)
  
  # form the initial estimator
  plugin1 <- mean(mu_hat1)
  plugin0 <- mean(mu_hat0)
  plugin <- plugin1 - plugin0
  psi_hat_1 <- mu_hat1 + val_idx/kappa_hat*(mu_hat1 + A/pi_hat*(Y - mu_hat1) - plugin1)
  psi_hat_0 <- mu_hat0 + val_idx/kappa_hat*(mu_hat0 + (1-A)/pi_hat*(Y - mu_hat0) - plugin0)
  psi_hat <- psi_hat_1 - psi_hat_0
  
  # form the augmentation term
  plugin1_ep <- mean(mu_hat1_ep)
  plugin0_ep <- mean(mu_hat0_ep)
  plugin_ep <- plugin1_ep - plugin0_ep
  psi_hat_1_ep <- (val_idx/kappa_hat - 1)*(mu_hat1_ep + Astar/pi_hat_ep*(Y - mu_hat1_ep)) 
  psi_hat_0_ep <- (val_idx/kappa_hat - 1)*(mu_hat0_ep + (1-Astar)/pi_hat_ep*(Y - mu_hat0_ep))
  psi_hat_ep <- psi_hat_1_ep - psi_hat_0_ep
  
  gamma_hat <- cov(psi_hat,psi_hat_ep)
  V_hat <- as.double(var(psi_hat_ep))

  # form the final estimator
  cv_eif <- psi_hat - gamma_hat/V_hat *as.double(psi_hat_ep)
  tau_cv <- mean(cv_eif)
  var_hat <-  var(cv_eif)/n
  
  cvmod <- lm(psi_hat ~ psi_hat_ep) 
  var_hat <- sandwich::vcovHC(cvmod, type = "HC3")[1,1] # robust variance estimator)

  return(list(
    ATE=tau_cv,
    EIF=cv_eif,
    var_hat=var_hat,
    ATE_val=mean(psi_hat),
    var_hat_val=var(psi_hat)/n
  ))
